...
var SessionIndex = class {
   ...
   */
  removeSessionsForStateFileNotIn(filePath, source, keepIds) {
    const toRemove = [];
...
function getAntigravityBrainRoot() {
  return path7.join(os5.homedir(), ".gemini", "antigravity", "brain");
}
async function discoverAntigravityConversationsAsync(override) {
  const brainRoot = override !== void 0 && override !== "" ? override : getAntigravityBrainRoot();
  let entries;
  try {
    entries = await fs7.promises.readdir(brainRoot);
  } catch {
    return [];
  }
  const results = await Promise.all(entries.map(async (entry) => {
    const conversationDir = path7.join(brainRoot, entry);
    try {
      const lstat = await fs7.promises.lstat(conversationDir);
      if (!lstat.isDirectory() || lstat.isSymbolicLink()) {
        return null;
      }
    } catch {
    ...
    try {
      const lstat = await fs7.promises.lstat(overviewFile);
      if (!lstat.isFile() || lstat.isSymbolicLink()) {
        return null;
      }
    } catch {
...
function resolveClaudeProjectsPath(override) {
  ...
  try {
    const vscode19 = require("vscode");
    const cfg = vscode19.workspace.getConfiguration("chatwizard");
...
function resolveAntigravityBrainPath(override) {
  if (override !== void 0 && override !== "") {
    return override;
  }
  try {
...
var WorkspaceScopeManager = class {
  ...
  _getOpenFolderPaths() {
    try {
      const vscode19 = require("vscode");
      return (vscode19.workspace.workspaceFolders ?? []).map(
  ...
  async initDefault(available) {
    ...
    if (openFolderPaths.length === 0) {
      ...
      ids = available.filter((ws) => openFolderPaths.includes(path9.normalize(ws.workspacePath).toLowerCase())).map((ws) => ws.id);
    }
...
async function calcWorkspaceSizeBytes(storageDir, source) {
  try {
    if (source === "copilot") {
      const dir = path9.join(storageDir, "chatSessions");
      const entries = await fs8.promises.readdir(dir);
      const sizes = await Promise.all(
        entries.filter((e) => e.endsWith(".jsonl")).map(async (f) => {
          try {
            return (await fs8.promises.stat(path9.join(dir, f))).size;
          } catch {
            return 0;
          }
        })
    ...
    if (source === "claude") {
      const entries = await fs8.promises.readdir(storageDir);
      const sizes = await Promise.all(
        entries.filter((e) => e.endsWith(".jsonl")).map(async (f) => {
          try {
            return (await fs8.promises.stat(path9.join(storageDir, f))).size;
          } catch {
            return 0;
          }
        })
    ...
    if (source === "cline" || source === "roocode") {
      const entries = await fs8.promises.readdir(storageDir, { withFileTypes: true });
      const sizes = await Promise.all(
        entries.filter((e) => e.isDirectory()).map(async (entry) => {
          const convFile = path9.join(storageDir, entry.name, "api_conversation_history.json");
          try {
            return (await fs8.promises.stat(convFile)).size;
          } catch {
            return 0;
          }
        })
    ...
    if (source === "cursor" || source === "windsurf") {
      const vscdb = path9.join(storageDir, "state.vscdb");
      try {
        return (await fs8.promises.stat(vscdb)).size;
      } catch {
        return 0;
      }
    }
    if (source === "aider") {
      const histFile = path9.join(storageDir, ".aider.chat.history.md");
      try {
        return (await fs8.promises.stat(histFile)).size;
      } catch {
        return 0;
      }
    }
...
async function countWorkspaceSessions(storageDir, source) {
  try {
    ...
    if (source === "cline" || source === "roocode") {
      ...
      const counts = await Promise.all(
        entries.filter((e) => e.isDirectory()).map(async (entry) => {
          const convFile = path9.join(storageDir, entry.name, "api_conversation_history.json");
          try {
            await fs8.promises.access(convFile);
            return 1;
          } catch {
            return 0;
          }
    ...
    if (source === "cursor" || source === "windsurf") {
      const vscdb = path9.join(storageDir, "state.vscdb");
      try {
        await fs8.promises.access(vscdb);
        return 1;
      } catch {
        return 0;
      }
    ...
    if (source === "aider") {
      const histFile = path9.join(storageDir, ".aider.chat.history.md");
      try {
        await fs8.promises.access(histFile);
        return 1;
      } catch {
        return 0;
      }
...
async function parseClineTask(taskDir, _maxLineChars, source = "cline") {
  const taskId = path10.basename(taskDir);
  const errors = [];
  const emptySession = {
    id: taskId,
    title: "Untitled Task",
  ...
  const conversationFile = path10.join(taskDir, "api_conversation_history.json");
  let apiEntries = [];
  try {
    const stat = await fs9.promises.stat(conversationFile);
    if (stat.size > MAX_FILE_BYTES) {
      errors.push(`api_conversation_history.json exceeds 50 MB size limit (${stat.size} bytes) \u2014 skipped`);
      return { session: emptySession, errors };
    }
    const raw = await fs9.promises.readFile(conversationFile, "utf-8");
    const parsed = JSON.parse(raw);
  ...
  let uiMessages = [];
  const uiFile = path10.join(taskDir, "ui_messages.json");
  try {
    const stat = await fs9.promises.stat(uiFile);
    if (stat.size <= MAX_FILE_BYTES) {
      const raw = await fs9.promises.readFile(uiFile, "utf-8");
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed)) {
        uiMessages = parsed;
      }
  ...
  if (!createdAt || !updatedAt) {
    ...
    try {
      fileSizeBytes = (await fs9.promises.stat(conversationFile)).size;
    } catch {
    }
  }
  return {
...
function extractItemText(item) {
  ...
  if (typeof item.richText === "string" && item.richText.trim()) {
  ...
  if (Array.isArray(item.parts)) {
    const fromParts = item.parts.map((p) => typeof p.text === "string" && p.text ? p.text : typeof p.content === "string" ? p.content : "").filter(Boolean).join("\n").trim();
    if (fromParts) {
      return fromParts;
    }
  }
...
var AISERVICE_SOURCE_NOTE = "This session was rebuilt from Cursor's aiService.prompts data. Cursor does not store full assistant replies in workspace state.vscdb for this build \u2014 only your prompts are available here. This is not a parse failure.";
function extractPromptComposerId(p) {
  if (!p || typeof p !== "object") {
    return void 0;
  }
  const o = p;
  const keys = ["composerId", "composerID", "composerSessionId", "sessionId", "chatId", "composer_id"];
  for (const k of keys) {
    const v = o[k];
...
function buildAiServiceFallbackSessions(vscdbPath, workspaceId, workspacePath, composers, promptsJson, generationsJson) {
  ...
  const buildMergedOrSingleComposerSession = (composer) => {
    const sid = composer?.composerId && composer.composerId.trim() ? composer.composerId.trim() : `${workspaceId}-cursor-aiservice`;
    const messages = [];
    for (let j = 0; j < flat.length; j++) {
      ...
      messages.push({
        ...
        timestamp: fp.unixMs !== void 0 ? new Date(fp.unixMs).toISOString() : void 0
      });
    }
    ...
    const createdAt = composer && typeof composer.createdAt === "number" && composer.createdAt > 0 ? new Date(composer.createdAt).toISOString() : messages.find((m) => m.timestamp)?.timestamp ?? (/* @__PURE__ */ new Date()).toISOString();
  ...
  const byComposer = /* @__PURE__ */ new Map();
  ...
  const stillFloating = [];
  for (const fp of flat) {
    if (!claimed.has(`${fp.index}`)) {
      stillFloating.push(fp);
    }
  }
  ...
  for (const [, arr] of byComposer) {
    for (const p of arr) {
      claimed2.add(`${p.index}`);
    }
  }
  const still2 = [];
  for (const fp of flat) {
    if (!claimed2.has(`${fp.index}`)) {
      still2.push(fp);
    }
  }
  if (still2.length > 0) {
  ...
  for (const c of ordered) {
    ...
    const createdAt = typeof c.createdAt === "number" && c.createdAt > 0 ? new Date(c.createdAt).toISOString() : messages.find((m) => m.timestamp)?.timestamp ?? (/* @__PURE__ */ new Date()).toISOString();
    const updatedAt = [...messages].reverse().find((m) => m.timestamp)?.timestamp ?? createdAt;
    results.push({
      session: {
        id: cid,
...
async function parseCursorWorkspace(vscdbPath, workspaceId, workspacePath) {
  ...
  if (rawValue === null || rawValue === void 0) {
    if (rawPrompts) {
      const fb2 = buildAiServiceFallbackSessions(
        vscdbPath,
        workspaceId,
        workspacePath,
        [],
      ...
      return fb2 && fb2.length > 0 ? fb2 : fatalResult("Missing usable composer data key in state.vscdb");
    }
  ...
  const allComposers = Array.isArray(composerData?.allComposers) ? composerData.allComposers : composerData?.composers;
  ...
  if (allComposers.length === 0) {
    const fb2 = rawPrompts ? buildAiServiceFallbackSessions(
      vscdbPath,
      workspaceId,
      workspacePath,
      [],
    ...
    return fb2 && fb2.length > 0 ? fb2 : [];
  }
  const composers = allComposers.slice(0, MAX_COMPOSERS);
  const perComposer = composers.map((composer) => {
    const composerId = typeof composer.composerId === "string" && composer.composerId ? composer.composerId : `${workspaceId}-${path11.basename(vscdbPath)}-unknown`;
    const errors = [];
    const conversation = Array.isArray(composer.conversation) ? composer.conversation : Array.isArray(composer.messages) ? composer.messages : [];
    const messages = [];
    for (const item of conversation) {
      ...
      messages.push({
        ...
        timestamp: typeof item.unixMs === "number" && item.unixMs > 0 ? new Date(item.unixMs).toISOString() : typeof item.timestamp === "number" && item.timestamp > 0 ? new Date(item.timestamp).toISOString() : typeof item.createdAt === "number" && item.createdAt > 0 ? new Date(item.createdAt).toISOString() : void 0
      });
    }
    const firstUserMsg = messages.find((m) => m.role === "user");
    let title;
    ...
    const createdAt = typeof composer.createdAt === "number" && composer.createdAt > 0 ? new Date(composer.createdAt).toISOString() : typeof composer.updatedAt === "number" && composer.updatedAt > 0 ? new Date(composer.updatedAt).toISOString() : messages.find((m) => m.timestamp)?.timestamp ?? (/* @__PURE__ */ new Date()).toISOString();
    const lastTimestampMsg = [...messages].reverse().find((m) => m.timestamp);
    const updatedAt = lastTimestampMsg?.timestamp ?? createdAt;
    return {
      session: {
  ...
  const fb = rawPrompts ? buildAiServiceFallbackSessions(
    vscdbPath,
    workspaceId,
    workspacePath,
    composers,
...
async function _walkAsync(currentDir, _rootDir, currentDepth, maxDepth, results) {
  let entries;
  try {
    entries = await fs10.promises.readdir(currentDir, { withFileTypes: true });
  } catch {
    return;
  }
  for (const entry of entries) {
    ...
    const historyFile = path12.join(currentDir, entry.name);
    try {
      const lstat = await fs10.promises.lstat(historyFile);
      if (!lstat.isFile() || lstat.isSymbolicLink()) {
        continue;
      }
    ...
    let configFile;
    const candidate = path12.join(currentDir, AIDER_CONFIG_FILENAME);
    try {
      const cfgStat = await fs10.promises.lstat(candidate);
  ...
  if (currentDepth < maxDepth) {
    await Promise.all(
      entries.filter((e) => e.isDirectory() && !e.isSymbolicLink()).map((e) => _walkAsync(path12.join(currentDir, e.name), _rootDir, currentDepth + 1, maxDepth, results))
    );
  }
}
...
function parseAiderHistory(info, maxLineChars = DEFAULT_MAX_LINE_CHARS2) {
  ...
  let rawContent;
  try {
    rawContent = fs11.readFileSync(info.historyFile, "utf8").replace(/\r\n/g, "\n");
  } catch (err) {
  ...
  let mtimeIso;
  try {
    const st = fs11.statSync(info.historyFile);
...
function parseAntigravityConversation(info) {
  ...
  let raw;
  try {
    raw = fs12.readFileSync(overviewFile, "utf-8");
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    errors.push(`Failed to read file: ${message}`);
    return { session: emptySession, errors };
...
var ChatWizardWatcher = class _ChatWizardWatcher {
  ...
  static _isSafeFilePath(resolvedBase, filePath) {
    try {
      ...
      return realPath.startsWith(resolvedBase + path13.sep) || realPath === resolvedBase;
    } catch {
      return false;
    }
  }
  static async _isSafeFilePathAsync(resolvedBase, filePath) {
    try {
      const realPath = await fs13.promises.realpath(filePath);
      return realPath.startsWith(resolvedBase + path13.sep) || realPath === resolvedBase;
    } catch {
      return false;
    }
  ...
  async start() {
    ...
    const indexAntigravity = cfg.get("indexAntigravity", true);
    if (!enabled) {
    ...
    if (indexCline) {
      ...
      clineWatcher.onDidDelete((uri) => {
        const taskId = path13.basename(path13.dirname(uri.fsPath));
    ...
    if (indexAntigravity) {
      ...
      antigravityWatcher.onDidDelete((uri) => {
        const conversationId = path13.basename(path13.dirname(path13.dirname(path13.dirname(uri.fsPath))));
  ...
  async buildInitialIndex(indexClaude, indexCopilot, indexCline, indexRooCode = true, indexCursor = true, indexWindsurf = true, indexAider = true, indexAntigravity = true) {
  ...
  async collectClaudeSessionsAsync(onProgress, selectedIds, _claudeBaseDirOverride) {
    ...
    try {
      let exists = false;
      try {
        exists = (await fs13.promises.stat(claudeProjectsDir)).isDirectory();
      } catch {
      }
      if (!exists) {
        return [];
      }
      const resolvedBase = await fs13.promises.realpath(claudeProjectsDir).catch(() => claudeProjectsDir);
      const projectDirEntries = await fs13.promises.readdir(claudeProjectsDir, { withFileTypes: true });
      ...
      const fileLists = await Promise.all(dirEntries.map(async (d) => {
        const projectPath = path13.join(claudeProjectsDir, d.name);
        try {
          const files = await fs13.promises.readdir(projectPath, { withFileTypes: true });
      ...
      const dirResults = await Promise.all(fileLists.map(async ({ projectPath, files }) => {
        ...
        for (const file of files) {
          const filePath = path13.join(projectPath, file.name);
  ...
  async collectAntigravitySessionsAsync(onProgress, _brainRootOverride) {
  ...
  collectClaudeSessions() {
    ...
    try {
      ...
      const resolvedBase = (() => {
        try {
          return fs13.realpathSync(claudeProjectsDir);
        } catch {
          return claudeProjectsDir;
        }
      })();
      const projectDirs = fs13.readdirSync(claudeProjectsDir, { withFileTypes: true });
      for (const projectDir of projectDirs) {
        if (!projectDir.isDirectory()) {
          continue;
        }
        const projectPath = path13.join(claudeProjectsDir, projectDir.name);
        try {
          const files = fs13.readdirSync(projectPath, { withFileTypes: true });
          for (const file of files) {
  ...
  async onClineFileChanged(uri) {
    const taskDir = path13.dirname(uri.fsPath);
    const taskId = path13.basename(taskDir);
  ...
  async onCursorFileChanged(uri) {
    const vscdbPath = uri.fsPath;
    const workspaceId = path13.basename(path13.dirname(vscdbPath));
    let workspacePath;
    try {
      const wsJson = path13.join(path13.dirname(vscdbPath), "workspace.json");
...
async function discoverClaudeWorkspacesAsync(override) {
  ...
  try {
    let exists = false;
    try {
      exists = (await fs14.promises.stat(claudeProjectsDir)).isDirectory();
    } catch {
    }
    if (!exists) {
      return [];
    }
    const entries = await fs14.promises.readdir(claudeProjectsDir, { withFileTypes: true });
    const results = [];
    for (const entry of entries) {
      if (!entry.isDirectory()) {
        continue;
      ...
      results.push({
        ...
        source: "claude",
        workspacePath,
        storageDir: path14.join(claudeProjectsDir, entry.name)
      });
    }
    return results;
  } catch {
...
function sourceCodiconId(source) {
  switch (source) {
    case "copilot":
      return "github";
    case "claude":
...
// src/ui/sourceBrandIcons.ts
var vscode2 = __toESM(require("vscode"));
function sourceBrandIconUris(source, extensionUri) {
  switch (source) {
    case "cline":
    case "roocode":
    case "cursor":
...
function sourceBrandIcon(source, extensionUri) {
  ...
  return brand ?? new vscode3.ThemeIcon(sourceCodiconId(source));
}
var SessionTreeItem = class extends vscode3.TreeItem {
  summary;
  pinned;
  constructor(summary, pinned = false, extensionUri) {
    super(summary.title || "Untitled Session", vscode3.TreeItemCollapsibleState.None);
    this.summary = summary;
    this.pinned = pinned;
    const workspaceName = path15.basename(summary.workspacePath ?? summary.workspaceId);
...
\u26A0 *This session has parse errors \u2014 some lines could not be read*` : "";
    const config = vscode3.workspace.getConfiguration("chatwizard");
    ...
    if (labelColor) {
      ...
      tooltip = new vscode3.MarkdownString(
...
${summary.userMessageCount} prompts \xB7 ${summary.assistantMessageCount} responses` + pinnedLine + interruptedLine + parseErrorsLine
    ...
    if (pinned) {
      this.iconPath = new vscode3.ThemeIcon("pinned");
      ...
      const red = new vscode3.ThemeColor("list.errorForeground");
      this.iconPath = new vscode3.ThemeIcon(sourceCodiconId(summary.source), red);
    } else if (summary.hasParseErrors) {
      const yellow = new vscode3.ThemeColor("list.warningForeground");
      this.iconPath = new vscode3.ThemeIcon(sourceCodiconId(summary.source), yellow);
    } else if (extensionUri) {
      this.iconPath = sourceBrandIcon(summary.source, extensionUri);
    } else {
      this.iconPath = new vscode3.ThemeIcon(sourceCodiconId(summary.source));
    }
    if (summary.hasParseErrors) {
      this.resourceUri = vscode3.Uri.from({ scheme: "chatwizard-warn", path: "/" + summary.id });
    }
...
var LoadingTreeItem = class extends vscode3.TreeItem {
  constructor() {
    super("Indexing sessions\u2026", vscode3.TreeItemCollapsibleState.None);
...
function compareBy(key, a, b) {
  switch (key) {
    ...
    case "workspace": {
      const wa = path15.basename(a.workspacePath ?? a.workspaceId);
      const wb = path15.basename(b.workspacePath ?? b.workspaceId);
...
var SessionParseWarningDecorationProvider = class {
  provideFileDecoration(uri) {
    if (uri.scheme === "chatwizard-warn") {
      return {
        badge: "\u26A0",
        color: new vscode3.ThemeColor("list.warningForeground"),
...
var SessionTreeProvider = class {
  ...
  hasActiveFilter() {
    ...
    return !!(f.title || f.dateFrom || f.dateTo || f.model || f.source || f.minMessages !== void 0 || f.maxMessages !== void 0 || f.hideInterrupted || f.onlyWithWarnings);
  }
  _matchesFilter(s) {
    const f = this._filter;
  ...
  _filterDescription() {
    ...
    if (f.source) {
      parts.push(`source:${friendlySourceName(f.source)}`);
    }
...
var CodeBlockGroupItem = class extends vscode4.TreeItem {
  sessionRef;
  constructor(group) {
    super(group.sessionTitle, vscode4.TreeItemCollapsibleState.Collapsed);
...
\`\`\``;
    ...
    this.tooltip = new vscode4.MarkdownString(
...
var CodeBlockLeafItem = class extends vscode4.TreeItem {
  block;
  sessionRef;
  constructor(block) {
    ...
    super(`${langLabel}: ${preview}`, vscode4.TreeItemCollapsibleState.None);
    this.block = block;
...
var CodeBlockLoadMoreItem = class extends vscode4.TreeItem {
  remaining;
  constructor(remaining) {
    super(`\u22EF Load more (${remaining} remaining)`, vscode4.TreeItemCollapsibleState.None);
    this.remaining = remaining;
...
var SessionWebviewPanel = class _SessionWebviewPanel {
  ...
  static show(context, session, searchTerm, scrollToCodeBlock, targetBlockMessageIndex, _targetBlockContent, targetBlockIdx, highlightContainer) {
    const config = vscode5.workspace.getConfiguration("chatwizard");
    ...
    if (existing) {
      existing.reveal(vscode5.ViewColumn.One);
...
(function() {
  ...
  static async _saveSelection(text, sessionTitle) {
    ...
    const defaultUri = vscode5.Uri.file(`${home}/${safe}-selection.md`);
...
function sourceButtonIcon(state) {
  if (state === "copilot") {
    return new vscode6.ThemeIcon("github");
  }
  if (state === "claude") {
    return new vscode6.ThemeIcon("hubot");
  }
  if (state === "antigravity") {
    return new vscode6.ThemeIcon("rocket");
  }
  return new vscode6.ThemeIcon("list-filter");
}
...
function msgTypeIcon(state) {
  if (state === "prompts") {
    return new vscode6.ThemeIcon("person");
  }
  if (state === "responses") {
    return new vscode6.ThemeIcon("hubot");
  }
  return new vscode6.ThemeIcon("comment-discussion");
}
  ...
  static show(context, index, engine) {
    ...
    function runSearch(value) {
      ...
      for (const result of response.results) {
        if (seenSessions.has(result.sessionId)) {
          continue;
        }
    ...
    quickPick.onDidAccept(() => {
      ...
      if (active && active.result) {
        ...
        vscode6.commands.executeCommand("chatwizard.openSession", active.summary, term);
      }
    });
    quickPick.onDidHide(() => {
      if (debounceTimer !== void 0) {
...
function defaultFolderUri() {
  ...
  const home = process.env["USERPROFILE"] ?? process.env["HOME"] ?? "/";
  return vscode7.Uri.file(home);
}
async function performExport(sessions) {
  if (sessions.length === 0) {
    vscode7.window.showInformationMessage("No sessions to export");
    return;
  }
  const modeItems = [
  ...
  const modePick = await vscode7.window.showQuickPick(modeItems, {
  ...
  if (modePick.id === "separate") {
    ...
    for (const session of sessions) {
      const fileUri = vscode7.Uri.joinPath(folder, `${safeFilename(session.title)}.md`);
      await vscode7.workspace.fs.writeFile(fileUri, Buffer.from(serializeSession(session), "utf8"));
    }
    vscode7.window.showInformationMessage(`Exported ${sessions.length} session(s) to ${folder.fsPath}`);
    return;
  }
  const saveUri = await vscode7.window.showSaveDialog({
    filters: { "Markdown": ["md"] },
    title: "Save combined export",
    defaultUri: vscode7.Uri.joinPath(defaultFolderUri(), "chatwizard-export.md")
  });
  if (!saveUri) {
    return;
  }
  await vscode7.workspace.fs.writeFile(saveUri, Buffer.from(serializeSessions(sessions, "combined"), "utf8"));
  await vscode7.window.showTextDocument(saveUri);
}
function registerExportCommands(context, index, getOrderedSummaries) {
  context.subscriptions.push(
    vscode7.commands.registerCommand("chatwizard.exportSession", async (item) => {
      const session = index.get(item.summary.id);
      if (!session) {
        vscode7.window.showErrorMessage(`Session not found: ${item.summary.id}`);
        return;
      }
      const filename = `${safeFilename(session.title)}.md`;
      const uri = await vscode7.window.showSaveDialog({
        defaultUri: vscode7.Uri.joinPath(defaultFolderUri(), filename),
        filters: { "Markdown": ["md"] },
      ...
      await vscode7.workspace.fs.writeFile(uri, Buffer.from(serializeSession(session), "utf8"));
      await vscode7.window.showTextDocument(uri);
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("chatwizard.exportAll", async () => {
      const sessions = index.getAllSummaries().map((s) => index.get(s.id)).filter((s) => s !== null && s !== void 0);
      await performExport(sessions);
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("chatwizard.exportSelected", async () => {
      ...
      const items = allSummaries.map((s) => ({
        ...
        description: `${path16.basename(s.workspacePath ?? s.workspaceId)} \xB7 ${s.updatedAt.slice(0, 10)}`,
        detail: `${friendlySourceName(s.source)} \xB7 ${s.messageCount} messages`,
        id: s.id
      }));
      const qp = vscode7.window.createQuickPick();
      qp.items = items;
      ...
      const sessions = picked.map((p) => index.get(p.id)).filter((s) => s !== null && s !== void 0);
      await performExport(sessions);
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("chatwizard.exportExcerpt", async (sessionId) => {
      const session = index.get(sessionId);
      if (!session) {
        vscode7.window.showErrorMessage(`Session not found: ${sessionId}`);
        return;
      }
      const visible = session.messages.filter((m) => m.content.trim() !== "");
      const assistantLabel = friendlySourceName(session.source);
      ...
      await vscode7.workspace.fs.writeFile(uri, Buffer.from(serializeSession(excerptSession), "utf8"));
      await vscode7.window.showTextDocument(uri);
    })
  );
}
...
var CodeBlocksPanel = class _CodeBlocksPanel {
  ...
  static show(context, index, engine) {
    ...
    if (_CodeBlocksPanel._panel) {
      _CodeBlocksPanel._panel.reveal(vscode8.ViewColumn.One);
    ...
    panel.webview.onDidReceiveMessage((message) => {
      if (message.command === "copy") {
        ...
        void vscode8.window.showInformationMessage("Code block copied to clipboard.");
...
var PromptLibraryPanel = class _PromptLibraryPanel {
  ...
  static show(context, index) {
    ...
    if (_PromptLibraryPanel._panel) {
      _PromptLibraryPanel._panel.reveal(vscode9.ViewColumn.One);
...
var PromptLibraryViewProvider = class {
  ...
  resolveWebviewView(webviewView, _context, _token) {
    ...
    webviewView.webview.onDidReceiveMessage((message) => {
      if (message.command === "copy") {
        void vscode10.env.clipboard.writeText(message.text ?? "");
        void vscode10.window.showInformationMessage("Prompt copied to clipboard.");
      } else if (message.command === "openSession" && message.sessionId) {
        void vscode10.commands.executeCommand("chatwizard.openSession", { id: message.sessionId }, message.searchTerm, message.highlightContainer);
      } else if (message.command === "openSettings") {
...
function countTokens(text, source) {
  ...
  if (source === "claude" || source === "antigravity") {
...
var AnalyticsPanel = class _AnalyticsPanel {
  ...
  static show(context, index) {
    if (_AnalyticsPanel._panel) {
      _AnalyticsPanel._panel.reveal(vscode11.ViewColumn.One);
    ...
    panel.webview.onDidReceiveMessage((msg) => {
      if (msg.type === "ready" && _AnalyticsPanel._panel) {
        ...
        void vscode11.commands.executeCommand("workbench.action.openSettings", "chatwizard");
      } else if (msg.command === "rescan") {
        void vscode11.commands.executeCommand("chatwizard.rescan");
      }
    }, void 0, context.subscriptions);
    panel.onDidDispose(() => {
  ...
  resolveWebviewView(webviewView, _context, _token) {
    ...
    webviewView.webview.onDidReceiveMessage((msg) => {
      if (msg.command === "openSession") {
        void vscode13.commands.executeCommand("chatwizard.openSession", { id: msg.sessionId });
        ...
        void vscode13.commands.executeCommand("workbench.action.openSettings", "chatwizard");
...
var TelemetryRecorder = class {
  ...
  constructor(storagePath) {
    this.filePath = path17.join(storagePath, "telemetry.jsonl");
  }
  ...
  record(event, properties) {
    ...
    try {
      fs15.mkdirSync(path17.dirname(this.filePath), { recursive: true });
      fs15.appendFileSync(this.filePath, JSON.stringify(entry) + "\n", "utf8");
    } catch {
    }
  }
  /**
  ...
  getEvents() {
    try {
      const content = fs15.readFileSync(this.filePath, "utf8");
  ...
  rotate() {
    try {
      let stat;
      try {
        stat = fs15.statSync(this.filePath);
      } catch {
        return;
      }
      ...
      if (needsRotation || hadOld || surviving.length < lines.length) {
        fs15.writeFileSync(this.filePath, surviving.map((l) => l.trim()).join("\n") + (surviving.length > 0 ? "\n" : ""), "utf8");
      }
    } catch {
    }
  }
...
function registerManageWorkspacesCommand(context, scopeManager, getWatcher, channel, index) {
  context.subscriptions.push(
    vscode14.commands.registerCommand("chatwizard.manageWatchedWorkspaces", async () => {
      ...
      function indexCountForIds(ids) {
        const dirs = ids.map((id) => {
          ...
          return path18.normalize(ws.storageDir);
        });
        return allSummaries.filter(
          (s) => dirs.some((dir) => path18.normalize(s.filePath).startsWith(dir + path18.sep))
        ).length;
      }
      ...
      for (const ws of allAvailable) {
        const key = path18.normalize(ws.workspacePath).toLowerCase();
      ...
      for (const key of allKeys) {
        const prefix = key.endsWith(path18.sep) ? key : key + path18.sep;
      ...
      for (const group of pathGroups.values()) {
        ...
        const cacheKey = path18.normalize(representative.workspacePath).toLowerCase();
        ...
        workspaceItems.push({
          ...
          label: path18.basename(representative.workspacePath),
      ...
      const picked = await new Promise((resolve) => {
        ...
        const qp = vscode14.window.createQuickPick();
        qp.canSelectMany = true;
        ...
        const openPaths = new Set(
          (vscode14.workspace.workspaceFolders ?? []).map((f) => path18.normalize(f.uri.fsPath).toLowerCase())
        );
        const currentWsItems = workspaceItems.filter((item) => {
          const itemPath = path18.normalize(item.workspacePath).toLowerCase();
          return [...openPaths].some(
            (op) => itemPath === op || itemPath.startsWith(op + path18.sep)
          );
        });
...
function registerPaletteCommands(context) {
  for (const cat of PALETTE_CATEGORIES) {
    context.subscriptions.push(
      vscode15.commands.registerCommand(cat.commandId, async () => {
        ...
        if (pick) {
          await vscode15.commands.executeCommand(pick.commandId);
        }
      })
    );
  }
...
function defaultVsCodeApi() {
  return {
    ...
    async loadModelWithProgress(task) {
      await vscode16.window.withProgress(
        { location: vscode16.ProgressLocation.Window, title: "Chat Wizard: loading AI model\u2026", cancellable: false },
        async (progress) => {
          await task((msg) => progress.report({ message: msg }));
        }
      );
    },
    async runIndexingProgress(task) {
      await vscode16.window.withProgress(
        { location: vscode16.ProgressLocation.Window, title: "Chat Wizard: indexing\u2026", cancellable: false },
        async (progress) => {
...
var SemanticIndexer = class {
  ...
  // ── search() ────────────────────────────────────────────────────────────
  async search(query, topK, minScore = SEMANTIC_MIN_SCORE, scope = "both") {
    if (!this._isReady) {
    ...
    return [...bestBySession.values()].sort((a, b) => b.score - a.score).slice(0, topK);
  }
  // ── dispose() ───────────────────────────────────────────────────────────
  dispose() {
    if (this._disposed) {
...
var FILE_VERSION = 2;
function makeKey(sessionId, role, messageIndex, paragraphIndex) {
...
function parseKey(key) {
  ...
  if (isNaN(messageIndex) || isNaN(paragraphIndex)) {
    return null;
  }
...
var SemanticIndex = class {
  ...
  // ── CRUD ───────────────────────────────────────────────────────────────
  add(sessionId, role, messageIndex, paragraphIndex, embedding) {
  ...
  // ── Search ─────────────────────────────────────────────────────────────
  search(queryEmbedding, topK, minScore = 0, scope = "both") {
    const results = [];
    for (const [key, embedding] of this._store) {
      if (scope !== "both") {
        ...
        if (key.includes(otherMarker) && !key.includes(roleMarker)) {
          continue;
        }
        const parsed = parseKey(key);
        if (!parsed || parsed.role !== scope) {
          continue;
        }
      }
  ...
  async save(filePath) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      buf.writeUInt32LE(keyBuf.byteLength, offset);
      offset += 4;
...
function sourceButtonIcon2(state) {
  if (state === "copilot") {
    return new vscode17.ThemeIcon("github");
  }
  if (state === "claude") {
    return new vscode17.ThemeIcon("hubot");
  }
  if (state === "antigravity") {
    return new vscode17.ThemeIcon("rocket");
  }
  return new vscode17.ThemeIcon("list-filter");
}
...
var SemanticSearchPanel = class {
  static show(_context, indexer, sessionIndex) {
    ...
    function scopeButtonIcon(s) {
      if (s === "user") {
        return new vscode17.ThemeIcon("comment");
      }
      if (s === "assistant") {
        return new vscode17.ThemeIcon("hubot");
      }
      return new vscode17.ThemeIcon("list-unordered");
    }
    function scopeButtonTooltip(s) {
      if (s === "both") {
        return "Scope: Both \u2014 click for My questions only";
      }
      if (s === "user") {
        return "Scope: My questions \u2014 click for AI responses only";
      }
    ...
    quickPick.buttons = [sourceButton, scopeButton];
    let debounceTimer;
    let lastRawResults = [];
    async function runSearch(value) {
      ...
      const minScore = vscode17.workspace.getConfiguration("chatwizard").get("semanticMinScore") ?? SEMANTIC_MIN_SCORE;
      quickPick.busy = true;
      try {
    ...
    quickPick.onDidAccept(() => {
      ...
      if (active && active.summary) {
        vscode17.commands.executeCommand("chatwizard.openSession", active.summary);
      }
    });
    quickPick.onDidHide(() => {
      if (debounceTimer !== void 0) {
...
async function activate(context) {
  const channel = vscode18.window.createOutputChannel("Chat Wizard");
  context.subscriptions.push(channel);
  const telemetry = new TelemetryRecorder(context.globalStorageUri.fsPath);
  const telemetryCfg = vscode18.workspace.getConfiguration("chatwizard");
  ...
  context.subscriptions.push(
    vscode18.window.registerWebviewViewProvider(PromptLibraryViewProvider.viewType, promptLibraryViewProvider)
  );
  const analyticsViewProvider = new AnalyticsViewProvider(index);
  context.subscriptions.push(
    vscode18.window.registerWebviewViewProvider(AnalyticsViewProvider.viewType, analyticsViewProvider)
  );
  const modelUsageViewProvider = new ModelUsageViewProvider(context, index);
  context.subscriptions.push(
    vscode18.window.registerWebviewViewProvider(ModelUsageViewProvider.viewType, modelUsageViewProvider)
  );
  const timelineViewProvider = new TimelineViewProvider(index, context);
  context.subscriptions.push(
    vscode18.window.registerWebviewViewProvider(TimelineViewProvider.viewType, timelineViewProvider)
  );
  context.subscriptions.push(
    vscode18.window.registerFileDecorationProvider(new SessionParseWarningDecorationProvider())
  );
  ...
  const semanticEmbeddingsUri = vscode18.Uri.joinPath(context.globalStorageUri, "semantic-embeddings.bin");
  ...
  if (vscode18.workspace.getConfiguration("chatwizard").get("enableSemanticSearch") ?? false) {
  ...
  const semanticListener = index.addTypedChangeListener((event) => {
    ...
    if (event.type === "batch") {
      ...
      void vscode18.workspace.fs.delete(semanticEmbeddingsUri).then(void 0, () => {
      });
      if (vscode18.workspace.getConfiguration("chatwizard").get("enableSemanticSearch") ?? false) {
        createAndInitSemanticIndexer();
      }
  ...
  context.subscriptions.push(
    vscode18.window.registerWebviewPanelSerializer("chatwizardAnalytics", {
  ...
  context.subscriptions.push(
    vscode18.window.registerWebviewPanelSerializer("chatwizardCodeBlocks", {
      async deserializeWebviewPanel(webviewPanel) {
        webviewPanel.webview.options = { enableScripts: true };
        ...
        webviewPanel.webview.onDidReceiveMessage((msg) => {
          if (msg.command === "copy") {
            void vscode18.env.clipboard.writeText(msg.text ?? "");
          } else if (msg.type === "ready") {
  ...
  context.subscriptions.push(
    vscode18.window.registerWebviewPanelSerializer("chatwizardPromptLibrary", {
      async deserializeWebviewPanel(webviewPanel) {
  ...
  context.subscriptions.push(
    vscode18.window.registerWebviewPanelSerializer("chatwizardSession3", {
      async deserializeWebviewPanel(webviewPanel) {
        webviewPanel.dispose();
      }
    })
...
Chat Wizard reads your Claude Code and GitHub Copilot chat history. Make sure the data paths are configured correctly.`;
  ...
  function syncContext() {
    ...
    void vscode18.commands.executeCommand("setContext", "chatwizard.sortKey", primary.key);
    void vscode18.commands.executeCommand("setContext", "chatwizard.sortDir", primary.direction);
    void vscode18.commands.executeCommand("setContext", "chatwizard.hasFilter", provider.hasActiveFilter());
    void vscode18.commands.executeCommand("setContext", "chatwizard.sessionGrouped", provider.isGrouped());
  }
  syncContext();
  function syncCbGroupContext() {
    void vscode18.commands.executeCommand("setContext", "chatwizard.cbGrouped", codeBlockProvider.isGrouped());
  }
  syncCbGroupContext();
  function savePins() {
  ...
  const dragDropController = {
    ...
    handleDrag(items, dataTransfer) {
      dataTransfer.set(
        ...
        new vscode18.DataTransferItem(items.map((i) => i.summary.id))
      );
    },
  ...
  const treeView = vscode18.window.createTreeView("chatwizardSessions", {
  ...
  const codeBlockTreeView = vscode18.window.createTreeView("chatwizardCodeBlocks", {
  ...
  function syncCbContext() {
    void vscode18.commands.executeCommand("setContext", "chatwizard.cbSortKey", codeBlockProvider.getSortMode());
    void vscode18.commands.executeCommand("setContext", "chatwizard.cbSortDir", codeBlockProvider.getSortDir());
  }
  syncCbContext();
  ...
  for (const mode of sortModes) {
    context.subscriptions.push(
      vscode18.commands.registerCommand(`chatwizard.sortBy${capitalise(mode)}`, () => applySort(mode))
    );
    context.subscriptions.push(
      vscode18.commands.registerCommand(`chatwizard.sortBy${capitalise(mode)}.asc`, () => applySort(mode))
    );
    context.subscriptions.push(
      vscode18.commands.registerCommand(`chatwizard.sortBy${capitalise(mode)}.desc`, () => applySort(mode))
    );
  }
  const cbSortModes = ["date", "workspace", "length", "title", "language"];
  for (const mode of cbSortModes) {
    context.subscriptions.push(
      vscode18.commands.registerCommand(`chatwizard.cbSortBy${capitalise(mode)}`, () => applyCbSort(mode))
    );
    context.subscriptions.push(
      vscode18.commands.registerCommand(`chatwizard.cbSortBy${capitalise(mode)}.asc`, () => applyCbSort(mode))
    );
    context.subscriptions.push(
      vscode18.commands.registerCommand(`chatwizard.cbSortBy${capitalise(mode)}.desc`, () => applyCbSort(mode))
    );
  }
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.configureSortOrder", async () => {
      ...
      for (let round = 0; round < 3; round++) {
        ...
        const keyPick = await vscode18.window.showQuickPick(items, {
        ...
        const dirPick = await vscode18.window.showQuickPick(dirItems, {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.filterSessions", async () => {
      ...
      const items = [
        ...
        },
        {
          ...
          description: current.source ? `current: ${friendlySourceName(current.source)}` : void 0
        },
        {
          id: "minMessages",
      ...
      const pick = await vscode18.window.showQuickPick(items, {
      ...
      if (pick.id === "_clear") {
        ...
        treeView.description = provider.getDescription();
        provider.refresh();
        void vscode18.commands.executeCommand("setContext", "chatwizard.hasFilter", false);
        return;
      }
      const newFilter = { ...current };
      if (pick.id === "title") {
        const val = await vscode18.window.showInputBox({
        ...
        const allSources = ["claude", "copilot", "cline", "roocode", "cursor", "windsurf", "aider", "antigravity"];
        const sourceItems = [
          { label: "$(close)  Show all sources", value: void 0 },
          ...allSources.map((s) => ({
        ...
        const chosen = await vscode18.window.showQuickPick(sourceItems, {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.filterCodeBlocks", async () => {
      ...
      if (pick.id === "language") {
        ...
        const allSources = ["copilot", "claude", "cline", "roocode", "cursor", "windsurf", "aider", "antigravity"];
        const sourceItems = [
          ...allSources.map((s) => ({
            label: `$(${sourceCodiconId(s)})  ${friendlySourceName(s)}`,
            source: s
        ...
        const sourcePick = await vscode18.window.showQuickPick(sourceItems, {
        ...
        const rolePick = await vscode18.window.showQuickPick(roleItems, {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.pinSession", (item) => {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.unpinSession", (item) => {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.loadMoreSessions", () => provider.loadMore())
  );
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.loadMoreCodeBlocks", () => codeBlockProvider.loadMore())
  );
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.toggleSessionGrouping", () => {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.enableSessionGrouping", () => {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.disableSessionGrouping", () => {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.toggleCbGrouping", () => {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.enableCbGrouping", () => {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.disableCbGrouping", () => {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.openSession", (summary, searchTerm, highlightContainer) => {
      const session = index.get(summary.id);
      if (!session) {
        vscode18.window.showErrorMessage(`Session not found: ${summary.id}`);
        return;
      }
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.openSessionFromCodeBlock", (ref) => {
      const session = index.get(ref.sessionId);
      if (!session) {
        vscode18.window.showErrorMessage(`Session not found: ${ref.sessionId}`);
        return;
      }
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.search", () => {
      telemetry.record("search.opened");
      SearchPanel.show(context, index, engine);
    })
  );
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.semanticSearch", () => {
      if (!semanticIndexer) {
        void vscode18.window.showInformationMessage(
          ...
          if (action === "Open Settings") {
            void vscode18.commands.executeCommand(
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.showCodeBlocks", () => {
      CodeBlocksPanel.show(context, index, codeBlockEngine);
    })
  );
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.showPromptLibrary", () => {
      PromptLibraryPanel.show(context, index);
    })
  );
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.showAnalytics", () => {
      AnalyticsPanel.show(context, index);
    })
  );
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.showTimeline", () => {
      void vscode18.commands.executeCommand("chatwizardTimeline.focus");
    })
  );
  context.subscriptions.push(
    vscode18.commands.registerCommand("chatwizard.rescan", () => {
      void vscode18.window.showInformationMessage(
        ...
        if (action === "Reload Window") {
          void vscode18.commands.executeCommand("workbench.action.reloadWindow");
        }
      });
    })
  );
  context.subscriptions.push(
    vscode18.workspace.onDidChangeConfiguration((e) => {
      ...
      if (e.affectsConfiguration("chatwizard.oldestSessionDate") || e.affectsConfiguration("chatwizard.maxSessions") || e.affectsConfiguration("chatwizard.indexCursor")) {
  ...
  context.subscriptions.push(
    vscode18.commands.registerCommand(
      ...
      async (item, allSelected) => {
        ...
        const sessions = items.map((i) => index.get(i.summary.id)).filter((s) => s !== null && s !== void 0);
        await performExport(sessions);
      }
    )
  );
