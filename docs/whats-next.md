...
| 11 | **Copilot Chronicle Phase 4 — work item & branch grouping** | New **"By Context"** tab grouping sessions by git branch and configurable work item pattern (`chatwizard.workItemPattern: "XLR-\\d{5}"`). New MCP tools `chatwizard_sessions_for_work_item` and `chatwizard_sessions_for_branch`. Chat participant file-tree rendering via `stream.filetree`. _Source: work-plan-extra-chat-data.md_ | "Show me all AI sessions for ticket ABC-1234" is the single most-requested enterprise workflow. Answers the question: _what did we ask AI while building this feature?_ | Pairs naturally with GitHub/JIRA and is a pre-requisite for the Team KB corporate feature. | M |
...
| No web/mobile access | Pieces, Recall | #27 Cloud Sync (P3), #56 Browser ext (P4) |
| No cross-machine sync | Pieces | #27 Cloud Sync (P3) |
| No JetBrains plugin | Pieces, Continue.dev | #55 (P4) |
| No team collaboration | DevChat, PromptLayer | #45–#53 Corporate tier (P4) |
| Search results unranked | Pieces, PromptLayer | #24 SQLite/FTS5 (P3) |
| No actual cost data | PromptLayer, Langfuse | #20 Prompt cost analysis (P2), #37 Post-session tips (P3) |
