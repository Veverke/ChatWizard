// Probe: run parseCursorGlobalDb logic using sql.js directly
...
async function main() {
    ...
    if (!tableCheck || tableCheck.length === 0) { console.log('No cursorDiskKV table'); db.close(); return; }
    console.log('cursorDiskKV table exists');

    const composerRows = db.exec("SELECT key, value FROM cursorDiskKV WHERE key LIKE 'composerData:%'");
    const bubbleRows = db.exec("SELECT key, value FROM cursorDiskKV WHERE key LIKE 'bubbleId:%'");
    const blobRows = db.exec("SELECT key, value FROM cursorDiskKV WHERE key LIKE 'agentKv:blob:%'");

    function rowsToArray(execResult) {
    ...
    const cRows = rowsToArray(composerRows);
    const bRows = rowsToArray(bubbleRows);
    const blRows = rowsToArray(blobRows);

    console.log(`composerRows: ${cRows.length}, bubbleRows: ${bRows.length}, blobRows: ${blRows.length}`);
