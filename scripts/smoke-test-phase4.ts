 ...
 *
 * Run with: tsx scripts/smoke-test-phase4.ts
 */
...
import { McpConfigHelper } from '../src/mcp/mcpConfigHelper';
import type { McpConfigTarget } from '../src/mcp/mcpConfigHelper';
...
function httpPost(url: string, body: unknown, headers: Record<string, string> = {}): Promise<{ status: number | undefined; body: string }> {
    return new Promise((resolve, reject) => {
...
async function main(): Promise<void> {
    ...
    const PORT = 7654;
    const TARGETS: McpConfigTarget[] = ['copilot', 'claude', 'cursor', 'continue', 'generic'];
