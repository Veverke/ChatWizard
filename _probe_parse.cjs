// Probe: run parseCursorGlobalDb and see what it returns
const path = require('path');
const os = require('os');

// Replicate the function from cursor.ts
const { openReadonlyDb } = require('./out/src/utils/sqliteDb');
...
async function main() {
    ...
    if (!db) { console.log('Failed to open DB'); return; }

    const tableCheck = db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='cursorDiskKV'");
    if (!tableCheck) { console.log('No cursorDiskKV table'); db.close(); return; }
    console.log('cursorDiskKV table exists');

    const composerRows = db.query("SELECT key, value FROM cursorDiskKV WHERE key LIKE 'composerData:%'");
    const bubbleRows = db.query("SELECT key, value FROM cursorDiskKV WHERE key LIKE 'bubbleId:%'");
    const blobRows = db.query("SELECT key, value FROM cursorDiskKV WHERE key LIKE 'agentKv:blob:%'");
    ...
    for (const row of composerRows) {
        ...
        if (rawBubbles.length === 0 && !useBlobMessages) {
            console.log(`  SKIP ${composerId}: no bubbles (${rawBubbles.length}) and no blob messages (${useBlobMessages})`);
            continue;
        }
